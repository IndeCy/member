...
<div class="row">
    <div class="col-md-12">
        ...
        <h1 class="page-subhead-line text-center">在此界面，您可以查看打赏记录。</h1>
    </div>
</div>
<div class="row">
	<div class="col-md-12">
		...
		<table class="table" id="table">
		  ...
		  <tbody>
			  <c:if test="${!empty myReward }">  
	             <c:forEach items="${myReward }" var="myReward">  
	                 <tr>  
	                     <td id="maccount${myReward.id}">${myReward.saccount }</td> 
	                     <td id="saccount${myReward.id}">${myReward.maccount }</td> 
	                     <td>${myReward.ddate }</td>  
	                     <td>${myReward.purpose }</td>  
	                     <td>${myReward.property }</td> 
	                 </tr>               
	             </c:forEach>  
	         </c:if>    
		  </tbody>
