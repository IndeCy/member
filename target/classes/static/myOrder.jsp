...
<div class="row">
    <div class="col-md-12">
        ...
        <h1 class="page-subhead-line text-center">在此界面，您可以查看已关注主播信息。</h1>
    </div>
</div>
<div class="row">
                            ...
                            <c:if test="${!empty star }">  
				                <c:forEach items="${star }" var="star">  
				                    <tr>  
				                        ...
				                        <a href="javascript:cancel('${star.account }')">取消订阅</a></td>
				                    </tr>               
				                </c:forEach>  
				            </c:if>
                        </tbody>
...
function cancel(account){  
	$.get("/DouyuMMS/member/cancleOrder?saccount="+account,function(data){ 
        if("success" == data.result){ 
            alert("取消成功！");  
            window.location.reload();  
        }else{  
